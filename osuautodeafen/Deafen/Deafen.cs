...
public class Deafen : IDisposable
{
    ...
    private static readonly Dictionary<Key, KeyCode> AvaloniaToSharpHookMap = new()
    {
        ...
        { Key.NumPad9, KeyCode.VcNumPad9 }, { Key.Add, KeyCode.VcNumPadAdd },
    ...
    private static readonly Dictionary<KeyCode, Key> SharpHookToAvaloniaMap =
        AvaloniaToSharpHookMap.ToDictionary(kv => kv.Value, kv => kv.Key);
    ...
    {
        return AvaloniaToSharpHookMap.GetValueOrDefault(key, KeyCode.VcUndefined);
    }

    private Key MapSharpHookKeyToAvaloniaKey(KeyCode keyCode)
    {
        return SharpHookToAvaloniaMap.GetValueOrDefault(keyCode, Key.None);
    }
