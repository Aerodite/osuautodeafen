...
public class ProgressIndicatorHelper
{
    ...
    public List<ObservablePoint> CalculateProgressIndicatorPoints(double completionPercentage, bool force = false)
    {
        ...
        var lineSeriesList = _chartManager.Series
            .OfType<LiveChartsCore.SkiaSharpView.LineSeries<ObservablePoint>>()
        ...
        var sortedPointsCache =
            new Dictionary<LiveChartsCore.SkiaSharpView.LineSeries<ObservablePoint>, List<ObservablePoint>>(lineSeriesList.Length);
        ...
        int steps = Math.Max(32, (int)(_chartManager.PlotView.Bounds.Width / 5));
...
public List<ObservablePoint> CalculateSmoothProgressContour(double completionPercentage, int steps = 400, bool force = false)
{
    var XAxes = _chartManager.XAxes;
    if (XAxes.Length == 0 || completionPercentage < 0 || completionPercentage > 100)
        return new List<ObservablePoint>();
    ...
    if (contour.Count < 2)
    {
        contour.Add(new ObservablePoint(progressPosition, 0));
    }
